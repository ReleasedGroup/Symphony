...
pull requests, and milestones), creates an isolated workspace for each issue, and runs a coding
agent session for that issue inside the workspace.
The GitHub repository is also the source of truth for code versioning (branches, commits, and PR
history).
...
- Symphony is a scheduler/runner and GitHub data reader.
- GitHub writes (issue state transitions, comments, PR links) are typically performed by the coding
...
3. `GitHub Tracker Client`
   - Fetches candidate issues in active states.
   - Fetches associated pull request and milestone metadata for issue context.
...
5. `Integration Layer` (GitHub adapter)
   - API calls and normalization for GitHub issue, pull request, and milestone data.
...
- GitHub API (`tracker.kind: github` in this specification version) for issues, pull requests, and
  milestones.
- GitHub repository source for code versioning metadata (branches/commits/PR refs).
...
- `identifier` (string)
  - Human-readable ticket key (example: `#123`).
- `title` (string)
...
- `branch_name` (string or null)
  - Branch metadata derived from GitHub issue/PR linkage if available.
- `url` (string or null)
- `milestone` (string or null)
  - GitHub milestone title when present.
- `labels` (list of strings)
  - Normalized to lowercase.
- `pull_requests` (list of pull request refs)
  - Each pull request ref contains:
    - `id` (string or null)
...
- `repo` (string)
  - Required for dispatch when `tracker.kind == "github"`.
- `milestone` (string or integer, optional)
  - Optional milestone selector for candidate issue filtering.
- `include_pull_requests` (boolean)
  - Default: `true`.
  - When `true`, include pull request metadata for candidate issues.
...
- `tracker.milestone`: string/integer, optional milestone filter
- `tracker.include_pull_requests`: boolean, default `true`
...
- GitHub is the source of truth for repository versioning (branches/commits/PR refs).
- This section does not assume a single fixed clone/sync strategy.
- Workspace preparation beyond directory creation (for example dependency bootstrap, checkout/sync,
  code generation) is implementation-defined and is typically handled via hooks.

### 9.3 Optional Workspace Population (Implementation-Defined)

The spec does not require a single built-in GitHub clone/sync strategy.
...
1. `fetch_candidate_issues()`
   - Return issues in configured active states for a configured GitHub repository.
   - Include associated pull request and milestone metadata when available.
...
- Candidate fetch includes issue records and linked pull request metadata when present
- Issue-state refresh query uses stable GitHub IDs where available
...
- `milestone` -> milestone title/string or null
- `pull_requests` -> normalized PR refs with number/state/url/branch refs when available
- `blocked_by` -> derived from dependency metadata or linked blocker relations when available
...
Symphony does not require first-class GitHub write APIs in the orchestrator.
...
1. `Workflow/Config Failures`
   ...
   - Unsupported tracker kind or missing tracker credentials/repository identity
...
- Filtering which GitHub repositories, milestones, labels, or issue subsets are eligible for
  dispatch so untrusted or out-of-scope tasks do not automatically reach the agent.
- Narrowing the optional `github_graphql` tool so it can only read or mutate data inside the
  intended repository scope, rather than exposing general workspace-wide tracker access.
...
- Milestone metadata is normalized when present
- Blockers are normalized from dependency/link metadata when available
- Labels are normalized to lowercase
- Issue state refresh by ID returns minimal normalized issues
- Issue state refresh query uses stable GitHub IDs as specified in Section 11.2
...
- Launch command uses workspace cwd and the platform shell:
  - Windows: `cmd.exe /d /s /c <codex.command>`
  - macOS/Linux: `/bin/bash -lc <codex.command>`
